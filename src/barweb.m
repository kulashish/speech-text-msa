function handles = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type, bw_legend2)
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)
				text(xdata(j)+.05,  -0.05*ymax*1.1, bw_legend(i), 'Rotation', 0, 'fontsize', 18, 'HorizontalAlignment', 'right');
                
                if(~isempty(bw_legend2))                    
                    text(xdata(j)+.05,  -0.1*ymax*1.2, bw_legend2(i), 'Rotation', 0, 'fontsize', 18, 'HorizontalAlignment', 'right');
                end
			end
		end
